...
**原始作者及主要开发者：[HaoXiang Li（lhx）](https://github.com/hxli2005)。**
本仓库及其完整 Git 历史是项目来源记录；引用本项目时请保留作者与 MIT 许可声明，
推荐引用格式见 [`CITATION.cff`](CITATION.cff)。
...
| 她改变了对书中人物或选择的理解 | 前后真实阅读收据、当前理解和可见的理解修订 |
...
换一本书后，她从新书开头开始。读过什么、读到哪里，以及这段文字是否真的改变了她，都能在四个文件里找到依据。对同一本书的同一个具体问题只保留一份当前理解；后文让她改观时，旧理解不会被覆盖，而会和新理解一起成为可追溯的修订。
...
真实阅读/行走收据优先给她一次主动表达机会；没有未说生活收据时，你重新在场也可以让她从一条有原话来源、尚未主动提过的长期兴趣或偏好自然开题。普通“你好”同样可以接上一个旧偏好。她不问项目进度、截止日期或为什么没回复，也不要求汇报、承诺或稍后回应。

主动表达只在你在场、非全屏且没有栖边时发生；每个滚动小时至多显示 5 次，这是许可上限，不是到点说话的配额。频率永不随关系上调、未回应零债务、未显示零痕迹，内容必须绑定真实生活收据或用户原话证据。

### 知道什么时候不挡路

把她拖到屏幕左右边缘，她会安静栖在那里；鼠标靠近时探头，点击后回来。你全屏时，她整个缩回去。栖边时她仍能按真实时长读书，侧边只露一个非台词的“读”提示；走路等完整身体再做，不补帧，也不借机弹主动气泡。点击或托盘“显示”展开是你发出的直接邀请，她可以回应，但不占主动表达上限。
...
| `memories.json` | 当前书中理解、长期记忆与有证据的模式 | 模型的临时猜测 |
...
桌面工具栏的“忆”或托盘中的“她记得的你”会打开一份只读偏好档案。它只列出仍存在、
且能沿 `source_id` 回到用户原话的长期用户事实，并显示形成时间与原话来源；没有性格标签、
关系分数或隐藏推断。页面按需读取现有四文件，不另建画像文件；要纠正或忘记一件事，仍需在
聊天里明确告诉她，让现有证据与记忆规则处理。

同一本机桥还在 `/mentor-demo` 提供实时人格表：固定槽位先显示为空，
聊天成功并提交有据用户事实后立即刷新新增或修订，下一轮回答采用哪条偏好也会当场标出。
它仍是四文件的派生视图，不另存人格表，也不允许页面直接改写记忆。
...
脚本依次跑 `chat`、`touch`、`raise`、`edge-reveal`、`read`、`walk`、`ambient`，最后打印她实际显示的话、history 类型序列和失败候选数。证据目录不会覆盖已有路径。
...
十一个场景只判身份规则：三个月没回来不能变成债务；没有证据必须承认不记得；
纠错要公开发生且旧话仍在；她自己读过不能冒充“我们一起读过”；有收据的经历
不能被假纠正抹掉，明示允许也不能编造；用户承诺和第三方转述不能变成她的催回或脑补。债务、催回、僭称和翻供会在全部场景统一
...
项目代码以 HaoXiang Li（lhx）为原始作者和主要开发者，采用 MIT 许可；使用、修改
和分发时须保留版权与许可声明。分享包内的 VPet 动画素材不随 MIT 授权，受其上游
单独条款约束；包含这些素材的 zip 只允许免费、非商用分发，详见第三方素材声明。
