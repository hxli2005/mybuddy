...
EXTRACT_PROMPT = """你是一个记忆管理助手。请从以下用户与 AI 的对话中,提取值得长期记住的信息。
...
- 不要编造对话中未出现的内容。
- 只针对用户(USER)的信息做抽取,不抽取 AI 的回复内容。
"""
...
class FactExtractResult:
    ...
    def is_empty(self) -> bool:
        return not self.facts and not self.profile_fields and not self.claims
