...
# ---------------------------------------------------------------------
# 本地壁钟时间解析(原 tools/reminder.py;提醒功能删除后只剩这一个消费者)
...
def _parse_wall_clock_time(value: str, *, now: datetime | None = None) -> datetime:
    """按本地壁钟解析时间文本。中文相对时间优先,失败后走 dateutil。"""
...
def _parse_chinese_relative_time(text: str, base: datetime) -> datetime | None:
    ...
    for word in ("大后天", "后天", "明天", "今天"):
        if word in text:
            date_offset = _DATE_OFFSETS[word]
            break
