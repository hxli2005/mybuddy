...
> 状态:`FINAL + ANIMATION REMEDIATION IMPLEMENTED`(2026-07-12)。
> 动画整改补充:`VPET_V1_ANIMATION_STATE_MACHINE.md`。§3 已按该补充改为单一 controller + renderer；自动验收、30 分钟 soak 与用户集中视觉复核均已通过，Gate A 为 PASS。

## 1. 项目结构

```
buddyshell/
  BuddyShell.csproj          # net8.0-windows, UseWPF；产品不加载 VPet.Core
  App.xaml(.cs)              # 单实例;异常兜底(任何未捕获异常不崩窗,状态点变红)
  MainWindow.xaml(.cs)       # 透明/置顶/无边框/可拖/记住位置/多屏安全
  Anim/IAnimationHost.cs     # IAnimationController/IAnimationRenderer 边界(见 §3)
  Anim/AnimationController.cs# 唯一状态仲裁器
  Anim/AnimationManifest.cs  # 显式 plan/phase/layer 素材表与启动校验
  Anim/AnimationTimeline.cs  # 单调时钟与共享多层 cursor
  Anim/FramePlayerHost.cs    # 纯 WPF renderer + 触摸命中
  # VPetCoreHost 已删除；spike 历史由 git 与 eval/acceptance/v1/spike/ 保留
...
- **紧凑主窗**:默认 `336×420 DIP`，只常驻宠物与 42 DIP 底部工具胶囊；聊天和五项食盘按需覆盖展开、互斥显示，不得通过纵向常驻面板扩大桌面占地。状态长文本省略并保留 ToolTip；175% DPI 下所有入口必须可见。
...
- `AnimationController` 唯一持有 baseline、transient、pending、queue、优先级、中断和恢复状态；窗口与网络回调不得直接播放目录。
- `AnimationManifest` 显式声明 A/B/C、loop/body、layer、z-index 和食物锚点；产品不再启发式扫描动画目录。
- `AnimationTimeline` 用一个单调时钟计算同一合成帧的全部 layer；`FramePlayerHost` 没有业务状态和独立动画 timer。
- 路线固定为 `AnimationController + FramePlayerHost`。旧 `VPetCoreHost` 源码和 Core NuGet 已从产品树移除；spike 历史由 git 与证据目录保留。
- Touch、Feed、Think、Work/Sleep 恢复及空 action 规则以动画整改规格 §8–§11 为准。
...
| EventAggregator.cs | TouchLayer.cs | 事件源从 VPet 事件改为 `ITouchSource.TouchDetected` |
