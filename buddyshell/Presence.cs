...
public sealed class Presence(ShellSettings settings) {
    public static bool AllowsAmbient(BodyPresence snapshot) => snapshot.Present && !snapshot.Fullscreen && snapshot.Surface == "full";
    public static bool MustDiscardAmbient(PendingBodyExpression? expression, BodyPresence? snapshot) => expression?.Kind == "ambient" && snapshot is not null && !AllowsAmbient(snapshot);
