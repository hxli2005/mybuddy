...
@pytest.mark.asyncio
async def test_agent_prefetches_search_context_for_news_question(tmp_path) -> None:
    cfg = Config()
    engine = init_db(str(tmp_path / "search_messages.db"))
    set_context(engine=engine, config=cfg)

    reg = ToolRegistry()

    @tool(name="web_search", description="mock search", registry=reg)
            ...
            "results": [
                {
                    ...
                    "snippet": "OpenAI announced a new model today.",
                }
            ][:max_results],
        }

    provider = ScriptedProvider([LLMResponse(text="我刚看了几条结果。", finish_reason="stop")])
    memory = _make_memory(engine, cfg, provider)
    logger = TrajectoryLogger(tmp_path / "traj")
    agent = Agent(
        provider=provider,
    ...
    result = await agent.run("查一下 OpenAI 最近有什么新闻")

    system_text = provider.calls[0]["system"]
    assert "## 外部资料检索" in system_text
    assert "OpenAI 发布新模型" in system_text
    assert "https://example.com/openai-news" in system_text
    assert result.tool_calls[0]["name"] == "web_search"
    assert result.tool_calls[0]["source"] == "backend_search_prefetch"
    assert result.trajectory.meta["search"]["result_count"] == 1
    ...
    with session_scope(engine) as s:
        ...
        assert "search_sources" in assistant.meta_json
...
async def test_agent_does_not_prefetch_search_for_personal_today_message(tmp_path) -> None:
    ...
    provider = ScriptedProvider([LLMResponse(text="我在。", finish_reason="stop")])
    memory = _make_memory(engine, cfg, provider)
    logger = TrajectoryLogger(tmp_path / "traj")
    agent = Agent(
        provider=provider,
    ...
    await agent.run("今天我好累,陪我一下")

    assert called == 0
    assert "## 外部资料检索" not in provider.calls[0]["system"]


@pytest.mark.asyncio
async def test_agent_prefetches_search_for_interest_topic_fact_detail(tmp_path) -> None:
    cfg = Config()
    engine = init_db(str(tmp_path / "interest_search_messages.db"))
    set_context(engine=engine, config=cfg)
    ...
    result = await agent.run("恋与深空那张卡剧情讲什么")

    system_text = provider.calls[0]["system"]
    assert "命中兴趣话题:恋与深空" in system_text
    assert result.tool_calls[0]["name"] == "web_search"
    assert result.tool_calls[0]["decision_topic"] == "恋与深空"
    assert result.search_sources[0]["title"] == "恋与深空资料页"
