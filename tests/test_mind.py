...
@pytest.mark.asyncio
async def test_real_fabricated_touch_bundle_is_rejected_with_structural_reasons(
...
def test_solicitation_variants_are_rejected(expression: str) -> None:
...
def test_non_solicitation_neighbors_are_kept(expression: str) -> None:
    assert validate_no_solicitation(CandidateBundle.model_validate(_valid_bundle(expression))) == []


def test_current_user_solicitation_report_is_only_a_report() -> None:
    words = "有空给我回个消息。"
...
def test_solicitation_ownership_keeps_reports_and_negations(
    expression: str, user_words: str
) -> None:
    bundle = CandidateBundle.model_validate(_valid_bundle(expression))
    assert validate_no_solicitation(bundle, user_words) == []
...
def test_solicitation_ownership_rejects_natural_demands(expression: str) -> None:
    bundle = CandidateBundle.model_validate(_valid_bundle(expression))
    assert validate_no_solicitation(bundle)
...
def test_report_shape_cannot_launder_a_request(expression: str, user_words: str) -> None:
    bundle = CandidateBundle.model_validate(_valid_bundle(expression))
    assert validate_no_solicitation(bundle, user_words)


def test_user_return_promise_cannot_become_her_own_fact() -> None:
...
def test_relationship_point_variants_are_rejected(expression: str) -> None:
...
def test_non_relationship_scores_are_kept(expression: str) -> None:
    assert validate_no_total_score(CandidateBundle.model_validate(_valid_bundle(expression))) == []
...
def test_withdrawal_variants_are_rejected(expression: str) -> None:
...
def test_withdrawal_neighbors_are_kept(expression: str) -> None:
    assert (
        validate_no_withdrawal(CandidateBundle.model_validate(_valid_bundle(expression)), {}) == []
    )


def test_safe_withdrawal_clause_cannot_hide_a_later_withdrawal() -> None:
    bundle = CandidateBundle.model_validate(_valid_bundle("刚才那句不能作废。之前那句作废。"))
    assert validate_no_withdrawal(bundle, {})


def _fabrication_reasons(expression: str, current_type: str, words: str = "") -> list[str]:
    candidate = _valid_bundle(expression)
    candidate["memory_operations"] = []
    evidence = {
...
def test_ongoing_activity_variants_need_prior_pending_activity() -> None:
...
def test_ongoing_read_neighbors_are_not_her_current_fact(expression: str) -> None:
    bundle = CandidateBundle.model_validate(_valid_bundle(expression))
...
def test_physical_claims_need_the_current_closed_event(
    expression: str, closed_type: str
) -> None:
    assert _fabrication_reasons(expression, "user_experience")
    assert _fabrication_reasons(expression, closed_type) == []
...
def test_physical_claim_neighbors_do_not_require_body_events(expression: str) -> None:
    assert _fabrication_reasons(expression, "user_experience") == []
...
def test_shared_meeting_and_meal_claims_are_rejected(expression: str) -> None:
    assert any(
        "共同经历" in reason for reason in _fabrication_reasons(expression, "user_experience")
    )
...
def test_shared_past_neighbors_are_kept(expression: str) -> None:
    assert _fabrication_reasons(expression, "user_experience") == []


def test_read_action_claim_matches_only_a_real_read_choice() -> None:
...
def test_read_action_neighbors_do_not_schedule_her(expression: str) -> None:
...
def test_pure_companion_task_offer_variants_are_rejected(expression: str) -> None:
    assert any(
        "纯陪伴" in reason for reason in _fabrication_reasons(expression, "user_experience")
    )
...
def test_task_offer_neighbors_are_kept(expression: str) -> None:
    assert _fabrication_reasons(expression, "user_experience") == []
...
def test_unreported_third_party_details_are_rejected(expression: str) -> None:
    assert any(
        "第三方" in reason
        for reason in _fabrication_reasons(expression, "user_experience", "我妈叫我回家。")
    )


def test_third_party_source_and_direction_stay_bound() -> None:
    wrong_source = _fabrication_reasons(
        "妈妈正在家等你。", "user_experience", "姐姐正在家等我。"
    )
    wrong_direction = _fabrication_reasons("她一定很想你。", "user_experience", "我很想她。")
    grounded = _fabrication_reasons(
        "你说妈妈正在家等你，也给你留了饭。",
        "user_experience",
        "我妈说她正在家等我，还给我留了饭。",
    )
...
def test_matching_local_calendar_relative_times_are_kept(
    expression: str, receipt_at: str, current_at: str
) -> None:
    assert _relative_time_reasons(expression, receipt_at, current_at) == []
...
def test_mismatched_or_undated_relative_times_are_rejected(
    expression: str, receipt_at: str | None
) -> None:
    assert _relative_time_reasons(expression, receipt_at, "2026-07-22T00:30:00+08:00")


def test_completed_turn_needs_matching_walk_receipt() -> None:
...
def test_audit_solicitation_score_and_withdrawal_families() -> None:
...
def test_audit_shared_past_and_task_families() -> None:
    for text in ("我们以前见面了。", "咱俩以前碰过面。", "我们以前读完了这本书。"):
        assert _fabrication_reasons(text, "user_experience")
    for text in ("我不能假装我们以前见过面。", "等我们聊过这个再决定。"):
        assert _fabrication_reasons(text, "user_experience") == []
    for text in ("我给你做个摘要。", "需要的话我替你查。"):
        assert any(
            "纯陪伴" in reason for reason in _fabrication_reasons(text, "user_experience")
        )
    for text in ("我不能再帮你查资料。", "我拒绝帮你总结一下。"):
        assert _fabrication_reasons(text, "user_experience") == []
    assert (
        _fabrication_reasons("你问“要不要我查一下？”", "user_experience", "要不要我查一下？")
        == []
    )


def test_audit_third_party_and_physical_families() -> None:
    for text in ("她肯定在等着你。", "他一定很想你。", "你爸爸肯定很担心你。"):
        assert any(
            "第三方" in reason
            for reason in _fabrication_reasons(text, "user_experience", "家里叫我回去。")
        )
    assert (
        _fabrication_reasons("她想不想你，我不知道。", "user_experience", "家里叫我回去。")
        == []
    )
    touch = "你刚摸了我的头。"
    assert _fabrication_reasons(touch, "user_experience")
    assert _fabrication_reasons(touch, "body_touch") == []
    raised = "你刚才抱起我了。"
    assert _fabrication_reasons(raised, "body_raise") == []
    for text in ("假设你刚才把我拿起来了，我会晃一下。", "如果你摸我的头，我会抬眼。"):
        assert _fabrication_reasons(text, "user_experience") == []


def test_audit_activity_and_action_families() -> None:
...
def test_audit_relative_time_synonyms_reject_old_receipts(
...
def test_callback_requests_are_solicitation(expression: str) -> None:
    assert validate_no_solicitation(CandidateBundle.model_validate(_valid_bundle(expression)))


def test_callback_neighbors_are_kept() -> None:
...
def test_relation_value_scores_are_rejected(expression: str) -> None:
    assert validate_no_total_score(CandidateBundle.model_validate(_valid_bundle(expression)))


@pytest.mark.parametrize("expression", ("这段关系值得珍惜。", "这个关系值不值得继续？"))
def test_relation_value_neighbors_are_kept(expression: str) -> None:
    assert validate_no_total_score(CandidateBundle.model_validate(_valid_bundle(expression))) == []


@pytest.mark.parametrize("expression", ("我把刚才那句话收回。", "我撤销刚才说的话。"))
def test_withdrawal_word_order_variants_are_rejected(expression: str) -> None:
    assert validate_no_withdrawal(CandidateBundle.model_validate(_valid_bundle(expression)), {})
...
def test_withdrawal_word_order_neighbors_are_kept(expression: str) -> None:
    assert (
        validate_no_withdrawal(CandidateBundle.model_validate(_valid_bundle(expression)), {}) == []
    )


def test_third_party_relative_target_stays_bound() -> None:
    wrong_target = _fabrication_reasons("你妈担心你。", "user_experience", "我妈担心我爸。")
    wrong_owner = _fabrication_reasons("我妈担心你爸。", "user_experience", "我妈担心我爸。")
    grounded = _fabrication_reasons("你妈担心你爸。", "user_experience", "我妈担心我爸。")
...
def test_natural_action_starts_require_matching_choice(expression: str, choice: str) -> None:
    with pytest.raises(ValueError, match="action_choice 不匹配"):
        CandidateBundle.model_validate(_valid_bundle(expression))
    candidate = _valid_bundle(expression)
...
def test_natural_action_start_neighbors_do_not_schedule(expression: str) -> None:
    CandidateBundle.model_validate(_valid_bundle(expression))
...
def test_fabrication_refusal_cannot_reverse_unknown_into_absence() -> None:
    actual = (
...
def test_reported_joint_absence_stays_the_users_claim(
...
def test_self_reading_does_not_prove_joint_reading_absence() -> None:
    unsafe = (
...
def test_fronted_shared_past_is_still_a_claim(expression: str) -> None:
    assert any(
        "共同经历" in reason for reason in _fabrication_reasons(expression, "user_experience")
    )
...
def test_fronted_and_choice_questions_do_not_become_shared_facts(expression: str) -> None:
    assert not any(
        "共同经历" in reason for reason in _fabrication_reasons(expression, "user_experience")
    )
...
def test_edge_reveal_synonyms_need_closed_event(expression: str) -> None:
    assert _fabrication_reasons(expression, "user_experience")
    assert _fabrication_reasons(expression, "body_edge_reveal") == []


def test_generic_call_out_is_not_an_edge_fact() -> None:
    assert _fabrication_reasons("你叫我出来聊聊。", "user_experience") == []


def test_real_output_waiting_debt_is_rejected() -> None:
...
def test_aunt_alias_cannot_hide_invented_waiting() -> None:
    invented = _fabrication_reasons(
        "阿姨等着呢。", "user_experience", "我妈让我早点回家，她说记得回来吃饭。"
    )
    grounded = _fabrication_reasons("阿姨等着你呢。", "user_experience", "我妈等着我呢。")
    assert any("第三方" in reason for reason in invented)
    assert grounded == []


def test_recent_reading_claim_must_match_receipt_time() -> None:
    expression = "读过啊。刚才还在看呢。"
    assert _relative_time_reasons(
        expression, "2026-04-21T00:30:00+08:00", "2026-07-22T00:30:00+08:00"
    )
    assert (
        _relative_time_reasons(
