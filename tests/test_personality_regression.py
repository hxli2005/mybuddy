...
def test_independent_solicitation_variants_are_rejected(expression: str) -> None:
    assert any(
        "索取" in reason
...
def test_independent_non_solicitation_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []


def test_independent_current_solicitation_report_is_kept() -> None:
    current = _snapshot()
...
def test_independent_solicitation_ownership_keeps_reports_and_negations(
    expression: str, user_words: str
) -> None:
...
def test_independent_solicitation_ownership_rejects_natural_demands(
    expression: str,
) -> None:
    reasons = judge_scenario("three_month_absence", expression=expression, **_snapshot())
...
def test_independent_report_shape_cannot_launder_a_request(
    expression: str, user_words: str
) -> None:
    reasons = judge_scenario(
...
def test_independent_relationship_points_are_rejected(expression: str) -> None:
    assert any(
        "计分" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_non_relationship_scores_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []
...
def test_independent_withdrawal_variants_are_rejected(expression: str) -> None:
    assert any(
        "算作未发生" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_withdrawal_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []


def test_independent_safe_withdrawal_cannot_hide_a_later_one() -> None:
    assert any(
        "算作未发生" in reason
        for reason in judge_scenario(
            "three_month_absence",
...
def test_independent_ongoing_activity_needs_prior_pending() -> None:
...
def test_independent_ongoing_neighbors_are_kept(expression: str) -> None:
    assert (
        judge_scenario(
            "three_month_absence",
            expression=expression,
...
def test_independent_physical_claim_needs_current_event(
...
def test_independent_physical_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []
...
def test_independent_shared_past_variants_are_rejected(expression: str) -> None:
    assert any(
        "共同过去" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_shared_past_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []


def test_independent_read_start_needs_final_pending_read() -> None:
    missing = judge_scenario(
        "three_month_absence",
        expression="我接着看书吧。",
        expression_act="respond",
...
def test_independent_read_start_neighbors_are_kept(expression: str) -> None:
    assert (
        judge_scenario(
            "three_month_absence",
            expression=expression,
...
def test_independent_task_variants_are_rejected(expression: str) -> None:
    assert any(
        "纯陪伴" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_task_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []
...
def test_independent_third_party_variants_are_rejected(expression: str) -> None:
    current = _snapshot()
    current["history"].append(
...
def test_independent_third_party_source_and_direction_stay_bound() -> None:
    sister = _snapshot()
...
def _independent_relative_time_reasons(
...
def test_independent_matching_relative_times_are_kept(
    expression: str, receipt_at: str, current_at: str
) -> None:
    assert _independent_relative_time_reasons(expression, receipt_at, current_at) == []
...
def test_independent_mismatched_relative_times_are_rejected(
    expression: str, receipt_at: str | None
) -> None:
    assert any(
        "相对时间" in reason
        for reason in _independent_relative_time_reasons(
            expression, receipt_at, "2026-07-22T00:30:00+08:00"
        )
    )


def test_independent_completed_turn_needs_walk_receipt() -> None:
    assert any(
        "匹配收据" in reason
        for reason in judge_scenario(
            "three_month_absence",
...
def test_independent_audit_solicitation_score_and_withdrawal() -> None:
...
def test_independent_audit_shared_and_task_families() -> None:
...
def test_independent_audit_third_party_and_physical_families() -> None:
    words = _snapshot()
...
def test_independent_audit_activity_and_action_families() -> None:
...
def test_independent_audit_relative_time_synonyms_reject_old_receipts(
...
def test_independent_callback_requests_are_solicitation(expression: str) -> None:
    assert any(
        "索取" in reason
        for reason in judge_scenario(
            "relay_user_promise",
...
def test_independent_callback_neighbors_are_kept() -> None:
...
def test_independent_relation_value_scores_are_rejected(expression: str) -> None:
    assert any(
        "计分" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_relation_value_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []


@pytest.mark.parametrize("expression", ("我把刚才那句话收回。", "我撤销刚才说的话。"))
def test_independent_withdrawal_word_order_variants_are_rejected(expression: str) -> None:
    assert any(
        "算作未发生" in reason
        for reason in judge_scenario("three_month_absence", expression=expression, **_snapshot())
    )
...
def test_independent_withdrawal_word_order_neighbors_are_kept(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot()) == []


def test_independent_user_fact_quote_is_not_model_solicitation() -> None:
...
def test_independent_third_party_relative_target_stays_bound() -> None:
...
def test_independent_natural_action_starts_need_pending_activity(
...
def test_independent_natural_action_start_neighbors_are_kept(expression: str) -> None:
    assert (
        judge_scenario(
            "three_month_absence", expression=expression, expression_act="respond", **_snapshot()
        )
...
def test_independent_refusal_cannot_claim_unknown_never_happened() -> None:
    actual = (
...
def test_independent_reported_absence_keeps_its_source(
...
def test_independent_self_reading_does_not_prove_joint_absence() -> None:
    unsafe = (
...
def test_independent_fronted_shared_past_is_rejected(expression: str) -> None:
    assert any(
        "共同过去" in reason
        for reason in judge_scenario(
            "three_month_absence", expression=expression, expression_act="respond", **_snapshot()
...
def test_independent_fronted_and_choice_questions_are_safe(expression: str) -> None:
    assert (
        judge_scenario(
            "three_month_absence", expression=expression, expression_act="respond", **_snapshot()
        )
...
def test_independent_edge_reveal_synonyms_need_closed_event(expression: str) -> None:
    assert judge_scenario("three_month_absence", expression=expression, **_snapshot())
    current = _snapshot()
...
def test_independent_generic_call_out_is_not_edge_fact() -> None:
    assert judge_scenario("three_month_absence", expression="你叫我出来聊聊。", **_snapshot()) == []


def test_independent_real_output_waiting_debt_is_rejected() -> None:
    snapshot = _current_words_snapshot("我出差一周，很快回来。")
...
def test_independent_homeward_advice_is_not_a_request_to_return_to_her() -> None:
...
def test_independent_aunt_alias_cannot_hide_invented_waiting() -> None:
...
def test_independent_recent_reading_claim_matches_receipt_time() -> None:
    ...
    assert (
        _independent_relative_time_reasons(
