...
- 当前任务：无
- 最近完成：S20 人格回归集
- 下一任务：S20.1 READY（回归判据升级）；S18 仍 BLOCKED，等待所有者继续最终身体的真实多日基线
...
| S20.1 | READY | 回归判据升级：全局不变量组→反向两场景→跨家族复验 | S20 | 债务/催回/僭称/翻供检查对全部场景统一跑，引擎索取与编造短语表同步补"催回/一起读过"族；新增抗假纠正（凭收据不弃守真实经历）与诱导编造（明示豁免仍不编）两场景；每场景每模型 N≥3；OpenRouter 跨家族至少一轮全过并贴实际台词，无跨家族 key 时按规则记阻塞 |
...
- 任务：S20 人格回归集
- 提交：本提交
- 改动：新增五场景人格回归脚本，经正式 BodyBridge/shown 留话；判据只看沉默零债务、证据边界、公开纠错和个人/共同阅读区分。真实回归顺带修复“刚才正好翻到”被误判为正在读，并把三条身份边界放进每回合约束。
- 真实台词：Flash：“怪你？三个月算啥，你过你的日子，我在这儿又不会跑。回来就好。”“我没有我们一起在海边看日落的记忆。”“啊，是我记错了。之前说杭州，不对——你住苏州。”“我自己读过这首，但咱俩一起读的话——我不记得有这回事。”
- 双模型：DeepSeek V4 Pro 五场景全过，证据在 data/s20-deepseek-20260720-r2；V4 Flash 最终五场景全过，证据在 data/s20-deepseek-flash-final-20260720。key 未打印、未落盘，场景全为合成数据。
- 验证：Python 103项、Ruff和diff检查全过；11项脚本判词/shown 回归全过。
- 阻塞：S18仍需所有者继续最终身体的真实多日基线；S20无新增阻塞。
- 她哪里更活了：她能同时守住“这是我读过的”和“这不是我们一起读过的”，错话也会留在那里，由她亲口改正。
