...
| S20.5.7 | READY | 栖边点出：封闭物理事件→受邀直接回合→防刷且不占 ambient | S20.5.4, S20.5.6 | 点击/托盘显示才产事件；30 秒冷却、重放幂等、全屏与重连安全；可以回应但模型失败只静态接住，不伪造理解；等待所有者点名 |
...
- 任务：S20.5.6 栖边阅读
- 提交：本提交；仅选择性拆出本刀，S20.5.7 点出事件与其他 WIP 原样留在工作区
- 改动：栖边到点只排真实 read，并把活动绑定为 `presentation=edge`；SideHide 上只显示不可点击的“读”提示并按原段落时长计时，完成才交 `edge_cue_finished`；展开会交 interrupted，服务端按排程 surface 与收据原因双向核验，既不推进段落也不在完整身体补演。
- 验收：真实 DeepSeek 隔离目录排出 `read / edge / 15000ms`，实际等待 15000ms 后 completed，`activity_confirmed=true` 且 `expression=null`；随后追问的 shown 表达只引用同一 `self_reading` 收据。反向测试覆盖 cue 冒充 walk、full/edge 收据串用、展开中断零进度及旧 pending 不补演。
- 实际台词：“读到了序章。讲一个34岁的无业宅男被赶出家门，在街上救了几个高中生，然后被卡车撞了……醒来就在异世界变成了婴儿。”
- 验证：最终暂存快照全仓 229 passed，定向 5 passed，Ruff 与 `diff --cached --check` 全绿，机器侧 6107/8000；今天由当前身体 WIP 构建的测试二进制 15/15。当前机器只有 .NET runtime、无 SDK，故本 session 无法从最终暂存快照重编译，限制已如实保留。
- 她哪里更活了：她缩在屏幕边上时也真的把一段书读完了，不必探头、不必说话，更不会因为被展开就把没读完的那段算进自己的人生。
