...
| S20.5.3 | READY | 生活时钟：30 分钟→5 分钟→真实 read/walk 仍按收据推进 | S20.4 | 只改心智生活步频，不引入说话调度器；时间跳跃、失败与中断不补帧不冒充经历；等待所有者点名领取 |
...
- 改动：删除正文“简短”硬约束；不索取改为按说话归属识别转述与新要求；过去书名按当前提问和完成收据匹配；自然不确定表达可用；第三方状态与人物方向同时绑定本次用户原话。
- 验收：生产与独立判据各自覆盖 M1 用户自己回来、M2 问 A 只答 A、M3 自然不确定、M6 第三方转述的真话与违规向；真实模型最终四向全部 accepted、shown 且 rule_failures=[]。
- 实际台词：M1“好，路上顺利。”；M2“不记得读过红楼梦。你问这个是想聊什么吗？”；M3“不记得有这件事。你是现在想聊海边的事吗？”；M6“嗯，你妈叫你回家吃饭呢。那你现在要回去了吗？”
- 验证：最终暂存快照全仓 218 passed；人格两文件 169 passed；M6 聚焦 20 passed；Ruff 全绿，diff --check 通过；仓库形状测试覆盖机器侧 8000 行目标。
- 她哪里更活了：她能把用户和家人的话接住后自然往下聊，也能坦然说自己记不清，却不会偷偷变成催回、编出谁在等，或拿别的一本书替眼前问题作答。
