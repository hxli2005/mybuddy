...
| S18 | BLOCKED | 多日真实轨迹→记忆/阅读/身体互相改变→人格验收 | S18.2 | 四审计问题由轨迹回答；停机不补帧、沉默零债务、世界阻力与技术故障严格分开；贴实际台词 |
...
- 任务：S18.2 栖边模式及标准后动画连续性修复
- 提交：本提交
- 闭环：拖到左右工作区边缘后只露 144 DIP；Raise 直接接 SideHide Main，悬停 Rise 退出后接回 Main Body，点击展开固定走 Main C 再回 idle；侧边与高度可重启恢复
- 真实断点修复：08:33–08:41 多次拖到屏幕边缘全部只落 body_raise、没有 edge_enter；根因是检测窗口边界与屏边“绝对距离”，按住人物中部时窗口会先越界而永远失配；现改为碰到或越过边缘即命中，并加入左右各半窗越界回归
- 动画断点修复：初版命中后先结束 Raise，必然播放落地 C；展开又直接换 idle，跳过 SideHide Main C。现复用旧版交互 follow-up/resume-body 语义，状态机回归逐相位钉住 Raise→Main、Rise C→Main B、Main B→Main C→idle
- 边界：ShellSettings 只保存纯呈现偏好；presence.surface 只有 full/edge，edge 不排 read/walk、不生成 ambient，同日未显示 ambient 直接丢弃且不写四文件；拖到侧边不是 body_raise 经历
- 真实窗口：最终 Release 在 175% DPI 下左栖边窗口为 -336..252、右栖边为 3588..4176，均恰好露 252 物理像素=144 DIP；日志实留 edge_enter left/right，前台全屏时实留 edge_fullscreen hidden=true
- 本刀实际运行：09:20 最终 Release 连回 8001 并恢复 right 栖边；此前 09:16 实际显示“读完了第一段。陶渊明这开头写得挺直白的……”；栖边切换本身仍不产生表达
- 回归：Python 82 passed，BuddyShell 12 passed，格式/diff 全绿；机器侧 4999 行；分享包 125.8 MiB、294 帧，授权文件仍在
- 产物：dist/MyBuddy-win-x64.zip；稳定文件名不再跟内部施工编号漂移
- 她哪里更活了：她不再先摔倒再藏起来，也不会带着藏身动作僵硬地回桌面；收起、探头、退回和走出来成为一口连贯的身体语言
