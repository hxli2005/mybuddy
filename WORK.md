...
- 任务：S18.9 按成熟开源仓库结构扩写 README
- 提交：本提交
- 改动：保留所有者写下的真实台词和“台词是生活副产品”主线，补齐快速扫读、差异证据表、生活分节、数据流、四文件、模型配置、隐私边界、源码验证、仓库导览、非目标、贡献和FAQ；同时纠正shown与直接回合重试的事实边界。
- 真实台词：README直接使用既有真实-key验收中显示过的“忙完啦？回来得正好，我正闲着。”“嗯？摸头干嘛，我又不是小猫。”“羁鸟恋旧林，池鱼思故渊……这两句读得人心口软了一下。”
- 验证：README中的内部链接、命令和模型配置与仓库现状核对；Markdown diff检查通过；仅文档与任务板变化，未修改运行代码。
- 阻塞：S18真实基线前仍须覆盖raise措辞“检查我还在不在？”这一用户动机猜测。
- 她哪里更活了：代码没有变；README现在不只让人听见她一句话，还能一路看清那句话怎样从身体、生活与证据中长出来。
