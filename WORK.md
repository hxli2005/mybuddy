...
- 当前写入者：无
- 当前任务：无
- 最近完成：S20.5.4 ambient 许可
- 下一任务：S20.5.5 READY，等待所有者点名领取
...
| S20.5.5 | READY | 本机策展阅读：私有 UTF-8 TXT→分段→开发读取且发行隔离 | S20.4 | 首块书名、空行分段、站点水印清理可核验；私有文本不入 Git、发行包、测试快照或公开演示，发行仍带公开样例；等待所有者点名 |
...
- 任务：S20.5.4 ambient 许可
- 提交：本提交；仅选择性拆出本刀，其他 S20.5 WIP 原样留在工作区
- 改动：read/walk 完成后可用真实收据提供 ambient；presence 从不在场恢复时只取最新未说生活收据；表达可安静，开口则只能 reflect 并引用该收据；滚动小时硬上限 5 只数 shown。
- 验收：连续 read→walk 均可产生新 ambient；第 5 次 shown 后即使塞入 `relationship_score=999` 仍被拦，第一次滑出 60 分钟后恢复；未 shown 不计数；模型选择安静时不写 pending/shared history；全屏、不在场、栖边均不展示。
- 实际台词：`self_walk` 收据 `walk_906b48ef3f1d4a8da03a613780d92d81` 后 12 秒 shown：“刚刚在窗口里走了走，从左边挪到右边。……嗯，就这样。”；`expression_act=reflect`，只引用该 walk 收据。
- 验证：最终暂存快照全仓 222 passed；当前 WIP 全仓 987 passed；Ruff 全绿；仓库形状/机器侧上限 5 passed；`diff --cached --check` 全绿；.NET 未运行（本刀未改身体端）。
- 她哪里更活了：她读完一段或真走完一步后，可以凭自己刚发生的生活自然开口，也可以不说；沉默不会变成她向用户追讨的债。
