...
- 任务：M3 导师要求对照与演示验收文档
- 提交：本提交（M3）
- 改动：新增单份导师交付文档，按会议纪要时间点逐条映射长记忆、主动性、个性化建模、过程可视化与 `personalize` 规范；给出当前字段表、闭环图、三段现场脚本、10 点验收和未完成边界。演示轨迹明确标为合成数据，不冒充真实用户画像；知识库、向量库与 RAG 只登记为未实现建议。
- 验收：文档 8 个主节、10 条可操作验收项；当前合成目录实际返回 4 条偏好，画像请求前后四文件 SHA-256 不变。
- 实际显示内容：“说到散步，你之前提过喜欢留意老建筑的窗户和门牌，我还挺好奇的——不同年代的门牌样式差别挺大的吧？”
- 验证：`diff --check` 通过；标题、合成数据披露、导师要求对照、字段规范、演示脚本、10 点验收和边界均已核对。
- 她哪里更活了：她对用户的了解、想起一件事的依据和主动开口的结果，现在能被导师沿一条证据链看懂，而不只剩一句“模型效果不错”。
