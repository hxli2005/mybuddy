...
- 当前写入者：无
- 当前任务：无
- 最近完成：导师演示分支并入 main 并按所有者裁决裁除演示交付物
...
- 任务：导师演示分支（M1–M5）快进并入 main，按所有者裁决只留功能
- 提交：本提交（合并收尾）
- 改动：M1–M5 功能全部并入——个性化主动话题、「她记得的你」窗口、`/mentor-demo` 实时人格表页面保留；演示汇报、导师要求对照文档、`mentor_demo.ps1`/`.cmd` 与合成夹具目录按所有者裁决删除，形状测试改为只校验保留页面；主工作区遗留的 `_generate_candidate` 重试 2→4 未提交实验按所有者裁决丢弃，重试语义仍与 DESIGN 第 4 条一致。
- 验收：删除后全仓无对已删文件的悬空引用（grep 核验）；`.tmp` 演示 worktree 与分支在合并后移除。
- 实际显示内容：本次为合并收尾，无新台词；M5 台词见 Git 中被替换的上一次交接。
- 验证：全仓 Python 1031 passed；Ruff 全绿；机器侧预算测试通过（≤8000）。
- 她哪里更活了：无新增；M1–M5 的活化（长期偏好开题、人格表可见生长）自本提交起正式生活在主线上。
