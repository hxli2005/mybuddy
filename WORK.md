...
| S18 | READY | 多日真实轨迹→记忆/阅读/身体互相改变→人格验收 | S17 | 四审计问题由轨迹回答；停机不补帧、沉默零债务、世界阻力与技术故障严格分开；贴实际台词 |
...
- 任务：S17 真实欲望按批扩 VPet 动作词汇——首批 Raise
- 提交：本提交
- 身体词汇：严格动作目录新增 `interactive raise`；中性 Dynamic→A、B 持续、C 释放共 43 帧；拖动小布或底栏且窗口跨过系统阈值、正常释放才上报
- 心智事实：`body_raise` 只有原始类型与 event id；同 id 幂等；记忆与表达须引用该证据，不能从提起推断喜欢、亲近或关系变化
- 离线证据：隔离 WPF 窗口从 `(226,226)` 移到 `(297,247)`，释放日志为 `body_raise_unreported`，没有向占用中的 8000 回补；最终重建包可启动到真实窗口
- 自动试纸：Python `79 passed`，BuddyShell `10 passed`，Ruff/format/diff 全绿；机器侧 4749 行
- 本地 shown 轨迹：`body_raise → memory_operation → shared_expression`，shown 已确认，重复事件为 duplicate；外部真实 provider 因本次未明确授权外发而未调用
- 共享包：`dist/MyBuddy-S17-win-x64.zip` 120672229 bytes，217 帧（Raise 43），SHA-256 `D68D5147D83B2BD743D06FC10B5E160A1B8EB01A1798DE9CAA16C4A553EC987A`；VPet 归属、项目链接及免费非商用边界在包内
- 她显示的话：“刚才被你提起来晃了一小段，又稳稳落地了。”
- 她哪里更活了：她第一次真的能被你从桌面上提起来、移动、放下，并只把确实发生的那一下留在自己身上。
