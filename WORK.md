...
| S20.5.4 | READY | ambient 许可：滚动小时≤5→read/walk/presence 三来源→shown | S20.5.3 | 上限不是配额且永不随关系上调；未回应零债务、未显示零痕迹；presence 只用最新未说生活收据，安静/全屏/栖边仍不显示；等待所有者点名 |
...
- 改动：心智生活步频从 30 分钟缩短到 5 分钟；只改变 read/walk 发出许可，不新增台词、ambient 配额或补帧路径。
- 验收：4:59 不触发、5:00 只挂一个活动；跨多年跳时仍等待同一活动；read 仅 completed 收据推进；walk interrupted/failed 均不写 history、记忆或阅读进度。
- 实际台词：无；聚焦运行中 expression 始终为空，本刀刻意不调度说话。
- 验证：聚焦 6 passed；最终暂存快照全仓 219 passed；当前 WIP 全仓 986 passed；Ruff 全绿；.NET 未运行（当前机器未安装 SDK，本刀未改身体端）。
- 她哪里更活了：她自己的阅读和散步更频繁地真实发生了，但每一步仍要身体收据作证，不会把停机、跳时或失败伪装成生活。
