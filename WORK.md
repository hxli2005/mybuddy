...
- 任务：M4 一键自动导师演示
- 提交：本提交（M4）
- 改动：新增 `mentor_demo.cmd/ps1` 与版本化合成记忆基线。每轮复制到独立 `data/mentor-demo-runs/<时间戳>`，验证画像零写入后临时关闭旧壳、构建、自动打开画像并展开来源、输入“你好”、等待真实模型与 `shown`、输出十点报告，再恢复原壳。四条例证覆盖表达习惯、决策偏好、科幻兴趣和生活观察，每条都有原话、记忆操作与当时回应；日常 `data/mini` 永不读取。WPF 的 `--mentor-demo` 只接受本轮独立壳目录，不写回日常设置。
- 验收：`-ValidateOnly` 六项全绿；`-HeadlessDemo` 用同一干净基线实际完成 `grounded_recall → shown`，绑定 `demo-source-old-buildings`；可见 WPF 由 Windows 快捷方式启动，规避 PowerShell/Codex 环境继承。
- 实际显示内容：“你好呀～突然想到你之前说散步时喜欢看老建筑的窗户和门牌，还特别留意那些后来被换过、补过的细节，这个习惯还挺有意思的，你是发现了什么特别的痕迹才开始注意的吗？”
- 验证：全仓 Python 1029 passed；本地 SDK 构建 0 警告/0 错误，身体测试 17/17；PowerShell 语法、fixture 结构、Ruff、`diff --check` 全绿；机器侧 7521/8000。
- 她哪里更活了：同一句普通“你好”现在能让她从一套完整过去中自己挑出最有辨识度的细节开口，而且导师能自动看到这句话从哪条原话长出来。
