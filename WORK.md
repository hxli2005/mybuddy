...
- 当前写入者：无
- 当前任务：无
- 最近完成：M5 实时人格表 Web 演示
- 下一任务：无；S21/S22 保持所有者阻塞
- 后续任务：S21/S22 已按所有者裁决预登记
- 工作区要求：独立导师演示分支，不带入主工作区未提交改动
...
| S20.5.8 | DONE | 分项集成：七刀提交→全套回归→固定双家族真实复验 | S20.5.1-S20.5.7 | 本地 Python/WPF/Ruff/diff/行数全过；DeepSeek V4 Flash + GPT-5 Mini 新目录全绿且报告可区分模型成功与静态失败；DESIGN/README 只事后如实记录并贴实际台词；final14 双家族 66/66，实际台词见最近交接 |
| S20.6 | DONE | 书中人格理解轨迹：真实阅读收据→唯一当下理解→退位理解入史→有据改观 | S18, S20.5.8 | **先只实现书，第二实例出现前不抽象。** 每个 scope 在 `memories.json` 只保留一个当前理解，含 `scope / formed_at`（引擎盖戳，禁倒填）`/ as_of_passage / view / uncertain / evidence_ids / perspective_ids / supersedes_event_id`；定位只用 `source + passage_index` 或可重建索引，不与动作 manifest 撞名、不生第五权威文件。理解修订须原子写入可见、可引用的一等人生事件 `understanding_revision`，保存退位理解全文、替代者、修订时间与真实修订依据；当前理解指向该事件，旧理解按地址冷读而非与当前版本并存或常驻热上下文；双向测试钉死 `understanding_revision` 可见、机器审计 `memory_operation` 不可见。`evidence_ids` 只承载原文与真实经历，`perspective_ids` 只承载使此处被留下的 seed/pattern，validator 强制二者交集为空；人格压缩只留下意外、改观、与自身倾向的张力、未解之问、后来获得的新义及自然淡去，读起来像百科摘要即失败。确定表达“这里确实写过”须引原文收据；“我印象里大概”须有相应印象/理解记录；“我现在更愿意理解成”须绑当前理解；“我读到前面时原本觉得”须绑退位理解；“记不清了，但留下的感觉是”须 `cannot_confirm` 过去细节并绑当前理解；无旧记录时必须切断过去断言，只能说“我不记得前面具体怎么想；现在读来……”，不得以“我好像一直觉得……”把无记录传记洗成模糊传记，cases 必含 `我好像一直觉得他是逃避 + 无旧理解记录` 的违规向反例。理解记录只能证明她曾经/现在怎样理解，永不证明书中事实，更不得作生活、共同或用户事实证据；事实措辞仍须原文收据。后续解冻顺序为书→她自己→用户与共同；用户/共同的意义修订必须绑定真实新经历，沉默和时间流逝不是修订证据——时间可以让她说记不清，不能让她得出“你不在乎我”；自然淡去只能降低可访问性与精确度，不得创造事实、负面意义或关系变化。验收须由真实长篇轨迹分别证明“索引证明书还在，轨迹证明她来过；轨迹不能替书作证”，并贴她实际说出的话。 |
| S20.7 | BLOCKED | 壳等待预算→覆盖 35–50 秒真实心智轮次→回答可靠显示与 shown | S20.6 | 代码与自动回归已完成；解锁条件：关闭当前占用旧 Debug 二进制的 `BuddyShell.exe`，用新构建完成一轮 35–50 秒真实聊天，回答实际显示并完成 shown；真实断线仍及时保留输入且不后台补发 |
| M1 | DONE | 长期用户记忆→问候/presence 机会→个性化主动话题→shown | S20.6 | 保留不索取/不编造/无总分/不撤回；问候或 presence 恢复可引用一条已持久化用户记忆自然开启新话题，不要求回应、不催项目；表达带记忆证据且 shown 前零共同历史，未回应零状态/记忆债务；无合格记忆时保持原行为 |
| M2 | DONE | 长期用户事实→按需只读派生→“她记得的你”窗口→原话与时间可见 | M1 | 不建第五文件、不推断人格、不加总分；身体仍只走 `/api/body/step`，打开页面才请求；仅展示有 `user_experience` 来源的当前 `user_fact`，关闭页面不写四文件 |
| M3 | DONE | 会议纪要要求→实现映射→个性化规范→10 点演示验收 | M2 | 单份导师交付文档明确已实现、现场操作、证据、合成数据披露与未完成边界；不把建议中的向量库/RAG冒充为当前实现 |
| M4 | DONE | 版本化干净合成记忆→一键启动→画像展开→问候主动话题→十点报告 | M3 | 每次复制独立运行目录，不污染日常或上次演示；四类最佳偏好例证均有原话/记忆操作/回应；自动演示走真实 WPF、真实模型和 shown，失败明确判红 |
| M5 | DONE | 固定人格表→聊天当轮写入→Web 即时高亮→偏好再次激活 | M4 | 同一导师分支；人格槽位为空时明确未形成，只有有原话来源的 user_fact 才能填入；聊天成功后立即刷新并展示新增/修订/无变化，查看页面零写入；自动演示证明新偏好当轮形成且后续 grounded_recall→shown |
...
- 任务：M5 实时人格表 Web 演示
- 提交：本提交（M5）
- 改动：同一本机桥新增轻量 `/mentor-demo` 页面；七个固定人格槽位直接投影四文件，有原话才填，空槽明确未形成。`user_fact` 可带受限 `profile_dimension`，旧事实只按原话保守归类；聊天成功后页面重读并高亮新增/修订/忘记或本轮激活，不建第五文件。干净基线改为三天三格，自动演示先用真实聊天形成生活兴趣，再以“你好”激活刚形成的偏好。
- 验收：`-ValidateOnly` 七项全绿；`-HeadlessDemo` 与可见默认浏览器均真实完成“3→4 条画像→grounded_recall→shown”，可见模式十点全绿；浏览器实查七个槽位、空白提示、原话、`source_id` 与最近激活正文。
- 实际显示内容：“你好呀。你之前说过散步时喜欢看老建筑的窗户和门牌,尤其留意后来被换过、补过的那些细节——这个我一直记着,挺特别的观察角度。”
- 验证：全仓 Python 1031 passed；本地 SDK 构建 0 警告/0 错误，身体测试 17/17；PowerShell 语法、Ruff、`diff --check` 全绿；机器侧 7542/8000。
- 她哪里更活了：用户刚把一个长期偏好说出口，导师就能看见人格表长出一格；下一声普通问候里，她又真的从刚长出的那一格想起了对方。
