...
| S20.5.6 | READY | 栖边阅读：真实 read 时长→最小非台词提示→completed 收据 | S20.5.3, S20.5.5 | SideHide 只演诚实可演形态；栖边禁 ambient/探头不变；展开中断不推进，完整身体不补演；等待所有者点名 |
...
- 改动：阅读源按 UTF-8（含 BOM）读入，以首个非空块作书名、空行分段并删除保守的整行站点水印；源码启动存在 `data/reading.local.txt` 时优先读取，缺失则仍用公开样例；分享构建只复制公开 `mybuddy/reading.txt`。
- 验收：本机私有读本由生产解析器读成 4320 段；两次 completed `self_reading` 的 `source=reading.local.txt`、标题与段号均正确，第二次产生 `reflect` ambient 且只引用该阅读收据；私有文件被 `/data/` 忽略、未被 Git 跟踪，分享脚本零次引用私有路径。
- 实际台词：私有读本相关原话只保存在被忽略的真实-key证据目录，不复制进 Git；本次 shown 的实际台词已在交付总结中原样贴出。
- 验证：定向 4 passed；最终暂存快照全仓 224 passed、机器侧 6004/8000；当前 WIP 全仓 989 passed；Ruff check 全绿；真实 DeepSeek 五条 shown、2 条结构失败候选保留后均成功重说；`diff --cached --check` 全绿；.NET 未运行（本刀未改身体端）。
- 她哪里更活了：她终于能从所有者真正为她挑的长篇里一段段往下读，同时那本私书不会被塞进分享给别人的她。
