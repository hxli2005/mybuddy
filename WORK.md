...
- 任务：M2 有证据的用户偏好档案
- 提交：本提交（M2）
- 改动：桌面工具栏“忆”和托盘“她记得的你”打开只读窗口；同一 `/api/body/step` 的互斥 `include_user_profile` 请求只返回当前有 `user_experience` 原话来源的 `user_fact`。页面展示原话、记录时间和来源 ID；不调用模型、不推进时间或 presence、不写四文件，也不创建画像文件。纠正与忘记仍走正常聊天证据路径。
- 验收：用 M1 真实轨迹按需读取出 4 条长期事实，依次为老建筑窗户与门牌、手绘地图细节取舍、科幻初次接触误解、鸟的落脚点；请求前后 `state/history/memories/failures` 四个 SHA-256 全部不变。
- 实际显示内容：“再记一条稳定偏好：我喜欢手绘地图，尤其爱看画地图的人如何取舍细节。”页面可展开到对应 `source_id` 与原话时间。
- 验证：全仓 Python 1028 passed；本地 SDK 构建 0 警告/0 错误，身体测试 17/17；Ruff、`diff --check` 全绿；机器侧 7448/8000。
- 她哪里更活了：她记得的“你”不再只藏在内部文件里；用户能当面看见每条记忆从哪句话长出来，同时她仍没有偷偷给用户打分或贴标签。
