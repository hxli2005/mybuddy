...
- 任务：S18.10 修复发布包已知交付缺口
- 提交：本提交
- 改动：分享包以pyproject版本为唯一来源；身体与心智版本在构建时互相核验；包内新增LICENSE和BUILD.txt，禁止PDB；输出版本化zip与SHA-256，旧候选只移入dist/previous而不删除；说明与仓库契约测试同步。
- 真实台词：本刀不改变人格或表达；最近真实验收台词仍是“忙完啦？回来得正好，我正闲着。”
- 验证：独立完整构建通过，包内LICENSE/BUILD齐全、PDB为0、两端均为0.1.0、sidecar哈希匹配；Python 87项、Ruff和身体12项全过。
- 阻塞：正式签名仍需所有者提供可信代码签名证书；S18真实基线前仍须覆盖raise措辞“检查我还在不在？”这一用户动机猜测。
- 她哪里更活了：她的台词没有因打包变活；但分享出去的她现在能明确证明自己是谁、从哪次提交来、有没有被改坏。
- 下一步：S18仍BLOCKED，等待所有者用最终身体做真实多日基线。
