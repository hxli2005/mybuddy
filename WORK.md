...
- 下一任务：S12 READY；字符预算长期记忆，领取后先让早期核心事实越过 tail-8
...
| S12 | READY | 字符预算记忆→核心常驻→情景填余量 | S11 | 超过八条后早期核心事实仍可见；核心超预算推动整合但不阻塞直接回复；不上BM25 |
| S13 | BLOCKED | 受约束种子→初始核心倾向→可被真实经历修正 | S12 | 行为试纸与两组张力进入核心记忆；表达样本只管渲染；无虚构传记、无固化亲密浓度 |
