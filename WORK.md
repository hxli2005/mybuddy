...
- 任务：M1 长期用户记忆驱动个性化主动话题
- 提交：本提交（M1）
- 改动：普通问候可选择一条有原话来源的长期兴趣/偏好自然开题；没有未说生活收据时，presence 恢复可从尚未主动提过的用户偏好生成一次可静默 ambient。模型只看到选中的一条记忆；表达必须 `grounded_recall` 绑定该证据，不得补写当前天气或环境，不催项目、进度、截止日期或回复，未回应不写状态或记忆债务。
- 验收：SOTA 模型在独立目录形成 19 条真实历史和 4 条长期用户事实；停止并重启后，普通“你好”在 5.8 秒引用手绘地图偏好，presence 恢复又实际显示两条有据 ambient。失败候选原文与拒因保留，修复“散步时你”误判后重新输入成功形成长期事实。
- 实际台词：“说到散步，你之前提过喜欢留意老建筑的窗户和门牌，我还挺好奇的——不同年代的门牌样式差别挺大的吧？”
- 验证：全仓 Python 1026 passed；本地 SDK 身体测试 16/16；Ruff、`diff --check` 全绿；机器侧 7311/8000。
- 她哪里更活了：她不再等用户把旧话题重新说一遍；一次普通问候或重新在场，就能从真实留下的偏好里自己挑一个具体角度开口。
